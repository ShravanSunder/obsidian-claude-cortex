...
export function createWriteEditBlock(
  ...
  const wrapperEl = parentEl.createDiv({ cls: 'cortex-write-edit-block' });
  wrapperEl.dataset.toolId = toolCall.id;

  // Header (clickable to collapse/expand)
  const headerEl = wrapperEl.createDiv({ cls: 'cortex-write-edit-header' });
  headerEl.setAttribute('tabindex', '0');
  headerEl.setAttribute('role', 'button');
  headerEl.setAttribute('aria-label', `${toolName}: ${shortenPath(filePath)} - click to expand`);

  // File icon
  const iconEl = headerEl.createDiv({ cls: 'cortex-write-edit-icon' });
  iconEl.setAttribute('aria-hidden', 'true');
  setIcon(iconEl, toolName === TOOL_EDIT ? 'file-pen' : 'file-plus');

  // Label: "Write: filename.md" or "Edit: filename.md"
  const labelEl = headerEl.createDiv({ cls: 'cortex-write-edit-label' });
  labelEl.setText(`${toolName}: ${shortenPath(filePath)}`);

  // Stats (will be updated when diff is ready): "+15 -20"
  const statsEl = headerEl.createDiv({ cls: 'cortex-write-edit-stats' });
  // Empty initially, populated when diff is computed

  // Status indicator (spinner while running)
  const statusEl = headerEl.createDiv({ cls: 'cortex-write-edit-status status-running' });
  statusEl.setAttribute('aria-label', 'Status: running');
  statusEl.createSpan({ cls: 'cortex-spinner' });

  // Content area (collapsed by default)
  const contentEl = wrapperEl.createDiv({ cls: 'cortex-write-edit-content' });

  // Initial loading state
  const loadingRow = contentEl.createDiv({ cls: 'cortex-write-edit-diff-row' });
  const loadingEl = loadingRow.createDiv({ cls: 'cortex-write-edit-loading' });
...
export function updateWriteEditWithDiff(state: WriteEditState, diffData: ToolDiffData): void {
  ...
  if (diffData.skippedReason === 'too_large') {
    const row = state.contentEl.createDiv({ cls: 'cortex-write-edit-diff-row' });
    const skipEl = row.createDiv({ cls: 'cortex-write-edit-binary' });
  ...
  if (isBinaryContent(originalContent) || isBinaryContent(newContent)) {
    const row = state.contentEl.createDiv({ cls: 'cortex-write-edit-diff-row' });
    const binaryEl = row.createDiv({ cls: 'cortex-write-edit-binary' });
  ...
  // Render diff content
  const row = state.contentEl.createDiv({ cls: 'cortex-write-edit-diff-row' });
  const diffEl = row.createDiv({ cls: 'cortex-write-edit-diff' });
...
export function finalizeWriteEditBlock(state: WriteEditState, isError: boolean): void {
  ...
  state.statusEl.className = 'cortex-write-edit-status';
  ...
  if (isError) {
    ...
    if (!state.diffLines) {
      state.contentEl.empty();
      const row = state.contentEl.createDiv({ cls: 'cortex-write-edit-diff-row' });
      const errorEl = row.createDiv({ cls: 'cortex-write-edit-error' });
...
export function renderStoredWriteEdit(parentEl: HTMLElement, toolCall: ToolCallInfo): HTMLElement {
  ...
  if (
    ...
    const diffLines = computeLineDiff(
      toolCall.diffData.originalContent,
  ...
  const statusEl = headerEl.createDiv({ cls: 'cortex-write-edit-status' });
  if (isError) {
  ...
  const row = contentEl.createDiv({ cls: 'cortex-write-edit-diff-row' });
  ...
  if (toolCall.diffData) {
    ...
    const doneEl = row.createDiv({ cls: 'cortex-write-edit-done-text' });
