...
export class InstructionModal extends Modal {
  ...
  constructor(app: App, rawInstruction: string, callbacks: InstructionModalCallbacks) {
    super(app);
  ...
  onOpen() {
    ...
    contentEl.addClass('cortex-instruction-modal');
    this.setTitle('Add Custom Instruction');

    // User input section (always visible)
    const inputSection = contentEl.createDiv({ cls: 'cortex-instruction-section' });
    const inputLabel = inputSection.createDiv({ cls: 'cortex-instruction-label' });
    inputLabel.setText('Your input:');
    const inputText = inputSection.createDiv({ cls: 'cortex-instruction-original' });
    inputText.setText(this.rawInstruction);

    // Main content section (changes based on state)
    this.contentSectionEl = contentEl.createDiv({ cls: 'cortex-instruction-content-section' });

    // Loading state
    this.loadingEl = this.contentSectionEl.createDiv({ cls: 'cortex-instruction-loading' });
    this.loadingEl.createDiv({ cls: 'cortex-instruction-spinner' });
    ...
    this.clarificationEl = this.contentSectionEl.createDiv({
      cls: 'cortex-instruction-clarification-section',
    });
    this.clarificationEl.style.display = 'none';
    this.clarificationTextEl = this.clarificationEl.createDiv({
      cls: 'cortex-instruction-clarification',
    });

    const responseSection = this.clarificationEl.createDiv({ cls: 'cortex-instruction-section' });
    const responseLabel = responseSection.createDiv({ cls: 'cortex-instruction-label' });
    responseLabel.setText('Your response:');

    this.responseTextarea = new TextAreaComponent(responseSection);
    this.responseTextarea.inputEl.addClass('cortex-instruction-response-textarea');
    ...
    this.confirmationEl = this.contentSectionEl.createDiv({
      cls: 'cortex-instruction-confirmation-section',
    });
    this.confirmationEl.style.display = 'none';

    // Refined instruction display/edit
    const refinedSection = this.confirmationEl.createDiv({ cls: 'cortex-instruction-section' });
    const refinedLabel = refinedSection.createDiv({ cls: 'cortex-instruction-label' });
    refinedLabel.setText('Refined snippet:');

    this.refinedDisplayEl = refinedSection.createDiv({ cls: 'cortex-instruction-refined' });
    this.editContainerEl = refinedSection.createDiv({ cls: 'cortex-instruction-edit-container' });
    this.editContainerEl.style.display = 'none';

    this.editTextarea = new TextAreaComponent(this.editContainerEl);
    this.editTextarea.inputEl.addClass('cortex-instruction-edit-textarea');
    this.editTextarea.inputEl.rows = 4;

    // Buttons (changes based on state)
    this.buttonsEl = contentEl.createDiv({ cls: 'cortex-instruction-buttons' });
    this.updateButtons();
  ...
  showClarificationLoading() {
    ...
    if (this.loadingEl) {
      this.loadingEl.querySelector('.cortex-instruction-spinner');
  ...
  private updateButtons() {
    ...
    const cancelBtn = this.buttonsEl.createEl('button', {
      text: 'Cancel',
      cls: 'cortex-instruction-btn cortex-instruction-reject-btn',
    ...
    if (this.state === 'clarification') {
      const submitBtn = this.buttonsEl.createEl('button', {
        text: 'Submit',
        cls: 'cortex-instruction-btn cortex-instruction-accept-btn',
      ...
      this.editBtnEl = this.buttonsEl.createEl('button', {
        text: 'Edit',
        cls: 'cortex-instruction-btn cortex-instruction-edit-btn',
