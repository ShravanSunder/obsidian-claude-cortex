...
function createMockPlugin(settings = {}) {
  const mockPlugin = {
    settings: {
      ...
      blockedCommands: {
        unix: ['rm -rf', 'rm -r /', 'chmod 777', 'chmod -R 777', 'mkfs', 'dd if=', '> /dev/sd'],
        windows: ['Remove-Item -Recurse -Force', 'Format-Volume'],
      },
      permissions: [],
      permissionMode: 'yolo',
      ...settings,
    ...
    getResolvedClaudeCliPath: vi.fn().mockReturnValue('/mock/claude'),
...
describe('CortexService', () => {
  ...
  beforeEach(() => {
    ...
    service = new CortexService(mockPlugin, createMockMcpManager());
  });
  ...
  describe('plan mode approvals', () => {
    it('includes revise feedback in ExitPlanMode response', async () => {
      ...
      const result = await (service as any).handleExitPlanModeTool(
        { plan: 'Plan draft' },
        'tool-1',
      );
    ...
    it('reads plan content from ~/.claude/plans with tilde expansion', async () => {
      ...
      (fs.readFileSync as import('vitest').Mock).mockReturnValue(planFromFile);

      service.setCurrentPlanFilePath('~/.claude/plans/plan.md');
      const callback = vi.fn().mockResolvedValue({ decision: 'cancel' });
  ...
  describe('findClaudeCLI', () => {
    ...
    afterEach(() => {
      (fs.existsSync as import('vitest').Mock).mockReset();
      (fs.statSync as import('vitest').Mock).mockReset();
    });
    ...
    it('should find claude CLI in ~/.claude/local/claude', async () => {
      ...
      (fs.existsSync as import('vitest').Mock).mockImplementation((p: string) => {
        return p === expectedPath;
      });
      (fs.statSync as import('vitest').Mock).mockReturnValue({ isFile: () => true });
    ...
    it('should use custom CLI path when valid file is specified', async () => {
      ...
      (fs.existsSync as import('vitest').Mock).mockImplementation((p: string) => p === customPath);
      (fs.statSync as import('vitest').Mock).mockReturnValue({ isFile: () => true });
    ...
    it('should fall back to auto-detection when custom path is a directory', async () => {
      ...
      (fs.existsSync as import('vitest').Mock).mockImplementation(
        (p: string) => p === customPath || p === autoDetectedPath,
      );
      (fs.statSync as import('vitest').Mock).mockImplementation((p: string) => ({
      ...
      expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('directory, not a file'));
    ...
    it('should fall back to auto-detection when custom path does not exist', async () => {
      ...
      (fs.existsSync as import('vitest').Mock).mockImplementation(
        (p: string) => p === autoDetectedPath, // Custom path does not exist
      );
      (fs.statSync as import('vitest').Mock).mockImplementation((p: string) => ({
    ...
    it('should fall back to auto-detection when custom path stat fails', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('not accessible'));
      consoleSpy.mockRestore();
    });
    ...
    it('should reload CLI path after cleanup', async () => {
      ...
      (fs.existsSync as import('vitest').Mock).mockImplementation((p: string) => p === firstPath);
      (fs.statSync as import('vitest').Mock).mockReturnValue({ isFile: () => true });
      ...
      (fs.existsSync as import('vitest').Mock).mockImplementation((p: string) => p === secondPath);
      (fs.statSync as import('vitest').Mock).mockReturnValue({ isFile: () => true });
  ...
  describe('transformSDKMessage', () => {
    ...
    it('should transform error messages', async () => {
      ...
      const errorChunk = chunks.find(
        (c) => c.type === 'error' && c.content === 'Something went wrong',
      );
      expect(errorChunk).toBeDefined();
    });
  ...
  describe('cleanup', () => {
    it('should call cancel and resetSession', () => {
      ...
      const resetSessionSpy = vi.spyOn(service, 'resetSession');
  ...
  describe('getVaultPath', () => {
    it('should return error when vault path cannot be determined', async () => {
      ...
      const errorChunk = chunks.find((c) => c.type === 'error' && c.content.includes('vault path'));
      expect(errorChunk).toBeDefined();
    });
  });
  ...
  describe('query with conversation history', () => {
    ...
    it('should rebuild history when session is missing but history exists', async () => {
      ...
      vi.spyOn(service as any, 'buildSDKUserMessage').mockImplementation(
        (prompt: string, images: any[]) => {
  ...
  describe('vault restriction', () => {
    ...
    it('should allow Read tool accessing files inside vault', async () => {
      setMockMessages([
        ...
        createAssistantWithToolUse(
          'Read',
          { file_path: '/test/vault/path/notes/test.md' },
          'read-inside',
        ),
    ...
    it('should block Write tool writing outside vault', async () => {
      setMockMessages([
        ...
        createAssistantWithToolUse(
          'Write',
          { file_path: '/tmp/malicious.sh', content: 'bad stuff' },
          'write-outside',
        ),
        { type: 'result' },
      ]);
    ...
    it('should allow Write tool writing to allowed export path', async () => {
      ...
      setMockMessages([
        ...
        createAssistantWithToolUse(
          'Write',
          { file_path: '/tmp/export.md', content: 'exported' },
          'write-export',
        ),
        { type: 'result' },
      ]);
    ...
    it('should block Write tool writing to context path under export path', async () => {
      ...
      setMockMessages([
        ...
        createAssistantWithToolUse(
          'Write',
          { file_path: '/tmp/workspace/out.md', content: 'blocked' },
          'write-context',
        ),
        { type: 'result' },
      ]);
    ...
    it('should allow Write tool writing to exact overlap path', async () => {
      ...
      setMockMessages([
        ...
        createAssistantWithToolUse(
          'Write',
          { file_path: '/tmp/shared/out.md', content: 'allowed' },
          'write-overlap',
        ),
        { type: 'result' },
      ]);
    ...
    it('should block Edit tool editing outside vault', async () => {
      setMockMessages([
        ...
        createAssistantWithToolUse(
          'Edit',
          { file_path: '/etc/hosts', old_string: 'old', new_string: 'new' },
          'edit-outside',
        ),
        { type: 'result' },
      ]);
    ...
    it('should allow Bash command writing to allowed export path via redirection', async () => {
      ...
      setMockMessages([
        ...
        createAssistantWithToolUse(
          'Bash',
          { command: 'cat ./notes/file.md > /tmp/out.md' },
          'bash-export',
        ),
        { type: 'result' },
      ]);
    ...
    it('should block Bash command writing to context path under export path', async () => {
      ...
      setMockMessages([
        ...
        createAssistantWithToolUse(
          'Bash',
          { command: 'echo hi > /tmp/workspace/out.md' },
          'bash-context-write',
        ),
        { type: 'result' },
      ]);
    ...
    it('should allow Bash command writing to allowed export path via -o', async () => {
      ...
      setMockMessages([
        ...
        createAssistantWithToolUse(
          'Bash',
          { command: 'pandoc ./notes/file.md -o /tmp/out.docx' },
          'bash-export-o',
        ),
        { type: 'result' },
      ]);
    ...
    it('should block Bash command copying from allowed export path into vault (write-only)', async () => {
      ...
      setMockMessages([
        ...
        createAssistantWithToolUse(
          'Bash',
          { command: 'cp /tmp/out.md ./notes/out.md' },
          'bash-export-cp',
        ),
        { type: 'result' },
      ]);
    ...
    it('should allow Bash commands with paths inside vault', async () => {
      setMockMessages([
        ...
        createAssistantWithToolUse(
          'Bash',
          { command: 'cat /test/vault/path/notes/file.md' },
          'bash-inside',
        ),
      ...
      const blockedChunk = chunks.find(
        (c) => c.type === 'blocked' && c.content.includes('outside the vault'),
      );
      expect(blockedChunk).toBeUndefined();
    });
    ...
    it('should block path traversal attempts', async () => {
      setMockMessages([
        ...
        createAssistantWithToolUse(
          'Read',
          { file_path: '/test/vault/path/../../../etc/passwd' },
          'read-traversal',
        ),
        { type: 'result' },
      ]);
    ...
    it('should block NotebookEdit tool writing outside vault', async () => {
      setMockMessages([
        ...
        createAssistantWithToolUse(
          'NotebookEdit',
          { notebook_path: '/etc/passwd', file_path: '/etc/passwd' },
          'notebook-outside',
        ),
        { type: 'result' },
      ]);
    ...
    it('should extract quoted and relative paths from bash commands', () => {
      const candidates = extractPathCandidates(
        'cat "../secret.txt" ./notes/file.md ~/vault/config',
      );
      expect(candidates).toEqual(
        expect.arrayContaining(['../secret.txt', './notes/file.md', '~/vault/config']),
      );
    });
  });
  ...
  describe('approval memory system', () => {
    ...
    it('should store session-scoped approved actions', async () => {
      ...
      await (service as any).approvalManager.approveAction(
        'Bash',
        { command: 'ls -la' },
        'session',
      );
      ...
      const isApproved = (service as any).approvalManager.isActionApproved('Bash', {
        command: 'ls -la',
      });
    ...
    it('should recognize permanently approved actions', async () => {
      ...
      const isApproved = (service as any).approvalManager.isActionApproved('Read', {
        file_path: '/test/file.md',
      });
      expect(isApproved).toBe(true);
    });
    ...
    it('should match Bash commands exactly', async () => {
      ...
      expect((service as any).approvalManager.isActionApproved('Bash', { command: 'ls -la' })).toBe(
        true,
      );

      // Different command should not be approved
      expect((service as any).approvalManager.isActionApproved('Bash', { command: 'ls -l' })).toBe(
        false,
      );
    });
    ...
    it('should match file paths with prefix', async () => {
      ...
      expect(
        (service as any).approvalManager.isActionApproved('Read', {
          file_path: '/test/vault/notes/file.md',
        }),
      ).toBe(true);
    ...
    it('should not match non-segment prefixes for file paths', () => {
      ...
      expect(
        (service as any).approvalManager.isActionApproved('Read', {
          file_path: '/test/vault/notes2/file.md',
        }),
      ).toBe(false);
    });
    ...
    it('should generate correct action descriptions', () => {
      ...
      expect(getActionDescription('Read', { file_path: '/test/file.md' })).toBe(
        'Read file: /test/file.md',
      );
      expect(getActionDescription('Write', { file_path: '/test/output.md' })).toBe(
        'Write to file: /test/output.md',
      );
      expect(getActionDescription('Edit', { file_path: '/test/edit.md' })).toBe(
        'Edit file: /test/edit.md',
      );
      expect(getActionDescription('Glob', { pattern: '**/*.md' })).toBe(
        'Search files matching: **/*.md',
      );
      expect(getActionDescription('Grep', { pattern: 'TODO' })).toBe(
  ...
  describe('safe mode approvals', () => {
    ...
    it('should allow and cache session approvals when user allows', async () => {
      const approvalCallback = vi.fn().mockResolvedValue('allow');
    ...
    it('should persist always-allow approvals and save settings', async () => {
      const approvalCallback = vi.fn().mockResolvedValue('allow-always');
      service.setApprovalCallback(approvalCallback);
      const canUse = (service as any).createUnifiedToolCallback('normal');
      const saveSpy = vi.spyOn(mockPlugin, 'saveSettings');

      const result = await canUse('Read', { file_path: '/test/file.md' }, {});

      expect(result.behavior).toBe('allow');
      expect(
        mockPlugin.settings.permissions.some(
          (a: any) => a.toolName === 'Read' && a.pattern === '/test/file.md',
        ),
      ).toBe(true);
    ...
    it('should deny when user rejects approval', async () => {
      const approvalCallback = vi.fn().mockResolvedValue('deny');
      service.setApprovalCallback(approvalCallback);
      const canUse = (service as any).createUnifiedToolCallback('normal');
    ...
    it('should deny and interrupt when approval flow errors', async () => {
      const approvalCallback = vi.fn().mockRejectedValue(new Error('boom'));
      service.setApprovalCallback(approvalCallback);
      const canUse = (service as any).createUnifiedToolCallback('normal');
          ...
          toolCalls: [
            {
              id: 'tool-1',
              name: 'Read',
              input: { file_path: '/test/vault/path/file.md' },
              status: 'completed' as const,
              result: 'file content',
            },
          ],
  ...
  describe('image prompt and hydration', () => {
    ...
    it('should hydrate images using existing data, cache, and file paths', async () => {
      ...
      vi.spyOn(imageCache, 'readCachedImageBase64').mockReturnValue('CACHE');

      (fs.existsSync as import('vitest').Mock).mockImplementation(
        (p: any) => p === '/test/vault/path/c.png',
      );
      (fs.readFileSync as import('vitest').Mock).mockReturnValue(Buffer.from('file-bytes'));
  ...
  describe('transformSDKMessage additional branches', () => {
    it('should transform tool_result blocks inside user content', () => {
      const sdkMessage: any = {
        type: 'user',
        message: {
          content: [{ type: 'tool_result', tool_use_id: 'tool-1', content: 'out', is_error: true }],
        },
      };

      const chunks = Array.from(transformSDKMessage(sdkMessage));
      expect(chunks[0]).toEqual(
        expect.objectContaining({ type: 'tool_result', id: 'tool-1', isError: true }),
      );
    });
    ...
    it('should prefer intendedModel over highest tokens when message.model is missing', () => {
      ...
      const chunksWithIntended = Array.from(
        transformSDKMessage(sdkMessage, { intendedModel: 'main-model' }),
      );
  ...
  describe('file hash hooks and diff data', () => {
    beforeEach(() => {
      (fs.existsSync as import('vitest').Mock).mockReset();
      (fs.statSync as import('vitest').Mock).mockReset();
      (fs.readFileSync as import('vitest').Mock).mockReset();
    ...
    it('captures original content and computes diff for small file', async () => {
      (fs.existsSync as import('vitest').Mock).mockReturnValue(true);
      (fs.statSync as import('vitest').Mock).mockReturnValue({ size: 10 });
      (fs.readFileSync as import('vitest').Mock)
    ...
    it('skips diff when original file is too large', async () => {
      (fs.existsSync as import('vitest').Mock).mockReturnValue(true);
      (fs.statSync as import('vitest').Mock).mockReturnValue({ size: 200 * 1024 });
  ...
  describe('remaining business branches', () => {
    ...
    it('yields error when persistent query fails to start', async () => {
      ...
      // Use setMockError from SDK mock for ESM compatibility
      setMockError(new Error('Session expired'));
    ...
    it('yields error for startup failures', async () => {
      ...
      // Use setMockError from SDK mock for ESM compatibility
      setMockError(new Error('Network down'));

      const chunks: any[] = [];
      for await (const c of service.query('Hi')) chunks.push(c);

      expect(chunks.some((c) => c.type === 'error' && c.content.includes('Network down'))).toBe(
        true,
      );
    ...
    it('handles image read errors and path resolution branches', () => {
      (fs.existsSync as import('vitest').Mock).mockReturnValue(true);
      (fs.readFileSync as import('vitest').Mock).mockImplementation(() => {
        throw new Error('boom');
      });
      const warnSpy = vi.spyOn(console, 'warn').mockImplementation(() => {});

      try {
        const base64 = readImageAttachmentBase64(
          mockPlugin.app,
          { filePath: 'x.png' } as any,
          '/test/vault',
        );
    ...
    it('returns continue for non-file tools in vault hook and null for unknown paths', async () => {
      ...
      const res = await hook.hooks[0](
        { tool_name: 'WebSearch', tool_input: {} } as any,
        't1',
        {} as any,
      );
    ...
    it('stores null original content when pre-hook stat fails', async () => {
      (fs.existsSync as import('vitest').Mock).mockReturnValue(true);
      (fs.statSync as import('vitest').Mock).mockImplementation(() => {
        throw new Error('boom');
      });
      const warnSpy = vi.spyOn(console, 'warn').mockImplementation(() => {});
      ...
      try {
        ...
        const preHook = createFileHashPreHook(vaultPath, originalContents);
        await preHook.hooks[0](
          { tool_name: 'Write', tool_input: { file_path: 'bad.md' } } as any,
          'tool-bad',
          {} as any,
        );
        ...
        const postHook = createFileHashPostHook(vaultPath, originalContents, pendingDiffData);
        await postHook.hooks[0](
          { tool_name: 'Write', tool_input: { file_path: 'bad.md' }, tool_result: {} } as any,
          'tool-bad',
          {} as any,
        );
        expect(pendingDiffData.get('tool-bad')).toEqual({
          filePath: 'bad.md',
    ...
    it('skips diff when post-hook lacks original entry or hits read error', async () => {
      (fs.existsSync as import('vitest').Mock).mockReturnValue(true);
      (fs.statSync as import('vitest').Mock).mockReturnValue({ size: 10 });
      (fs.readFileSync as import('vitest').Mock).mockReturnValueOnce('new');
      const warnSpy = vi.spyOn(console, 'warn').mockImplementation(() => {});
      ...
      try {
        ...
        const postHook = createFileHashPostHook(vaultPath, originalContents, pendingDiffData);
        await postHook.hooks[0](
          { tool_name: 'Write', tool_input: { file_path: 'no-orig.md' }, tool_result: {} } as any,
          'tool-no-orig',
          {} as any,
        );
        expect(pendingDiffData.get('tool-no-orig')).toEqual({
          filePath: 'no-orig.md',
          skippedReason: 'unavailable',
        });
        ...
        await postHook.hooks[0](
          { tool_name: 'Write', tool_input: { file_path: 'err.md' }, tool_result: {} } as any,
          'tool-read-err',
          {} as any,
        );
        expect(pendingDiffData.get('tool-read-err')).toEqual({
          filePath: 'err.md',
          skippedReason: 'unavailable',
        });
        expect(warnSpy).toHaveBeenCalledWith(
    ...
    it('marks too_large when post-hook sees large new file', async () => {
      ...
      const preHook = createFileHashPreHook(vaultPath, originalContents);
      await preHook.hooks[0](
        { tool_name: 'Write', tool_input: { file_path: 'large.md' } } as any,
        'tool-large',
        {} as any,
      );
      ...
      const postHook = createFileHashPostHook(vaultPath, originalContents, pendingDiffData);
      await postHook.hooks[0](
        { tool_name: 'Write', tool_input: { file_path: 'large.md' }, tool_result: {} } as any,
        'tool-large',
        {} as any,
      );

      expect(pendingDiffData.get('tool-large')).toEqual({
        filePath: 'large.md',
